...
get '/getUpdates' do
	...
	jsonHash = {}
	jams = Jam.all
	jams = jams.where(:id => Jam.select(:id).where(:id => Collaboration.select(:jam_id).where(:user_id => params[:userID])).exclude(:id => Song.select(:jam_id).where(:user_id => params[:userID])))
	jams.each{ |jam|
